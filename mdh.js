...
function playSong(songTitle, songUrl, thumb, artist) {
   // Update Player UI (New Model)
   const singerCover = document.querySelector('.player-cover__item');
   if(singerCover) {
      // Use double quotes for URL to prevent issues with spaces/special chars
      singerCover.style.backgroundImage = `url("${thumb}")`;
   }
   
   const titleEl = document.querySelector('.album-info__track');
   const artistEl = document.querySelector('.album-info__name');
   if(titleEl) titleEl.innerText = songTitle;
   if(artistEl) artistEl.innerText = artist;

   // Support for secondary displays
   const playingThumb = document.getElementById('playingThumb');
   if(playingThumb) playingThumb.src = thumb;
