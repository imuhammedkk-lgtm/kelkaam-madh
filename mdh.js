...
/* Helper to clean and normalize titles/filenames for flexible matching (supports English & Malayalam) */
function normalizeMediaName(str) {
   if (!str) return "";
   return str
      .toLowerCase()
      .replace(/\.(mp3|wav|ogg|m4a|aac|jpg|jpeg|png|webp|avif|gif)$/i, '')
      .replace(/[^a-z0-9\u0d00-\u0d7f]+/gi, ' ')
      .trim();
}

/* Check similarity or prefix match between audio filename and image filename */
function isMediaSimilar(audioName, imageName) {
   const normAudio = normalizeMediaName(audioName);
   const normImage = normalizeMediaName(imageName);

   if (!normAudio || !normImage) return false;
   if (normAudio === normImage) return true;

   // Prefix match: audio starts with image name or image starts with audio name
   if (normAudio.startsWith(normImage) || normImage.startsWith(normAudio)) return true;

   // Word containment match: image contains audio core or vice-versa
   if (normAudio.includes(normImage) || normImage.includes(normAudio)) return true;

   // First word / core phrase match
   const audioWords = normAudio.split(' ').filter(w => w.length > 2);
   const imageWords = normImage.split(' ').filter(w => w.length > 2);
   if (audioWords.length > 0 && imageWords.length > 0) {
      if (audioWords[0] === imageWords[0]) return true;
   }

   return false;
}

/* Helper to resolve matched audio and image files flexibly */
function getSingerSongMedia(song, singerName) {
   if (typeof song === 'string') {
      song = { title: song };
   }

   const baseAudioName = song.filename || song.file || song.audio || song.title;
   const baseImageName = song.imageName || song.imgName || song.image || baseAudioName;

   const singerFolder = `singers/${singerName}`;
   const defaultUrl = song.url || `${singerFolder}/audio/${baseAudioName}.mp3`;
   const defaultImg = song.img || `${singerFolder}/images/${baseImageName}.jpg`;

   return {
      title: song.title || baseAudioName,
      url: defaultUrl,
      img: defaultImg,
      lyrics: song.lyrics || null,
      audioBase: baseAudioName,
      imageBase: baseImageName
   };
}

/* Smart fallback chain for image loading (jpg -> png -> webp -> jpeg -> avif -> singer profile pic) */
function handleImageError(imgEl, singerName, baseName, defaultAvatar) {
   const state = imgEl.dataset.triedExts || "start";
   const singerFolder = `singers/${singerName}/images`;
   
   if (state === "start") {
      imgEl.dataset.triedExts = "png";
      imgEl.src = `${singerFolder}/${baseName}.png`;
   } else if (state === "png") {
      imgEl.dataset.triedExts = "webp";
      imgEl.src = `${singerFolder}/${baseName}.webp`;
   } else if (state === "webp") {
      imgEl.dataset.triedExts = "jpeg";
      imgEl.src = `${singerFolder}/${baseName}.jpeg`;
   } else if (state === "jpeg") {
      imgEl.dataset.triedExts = "avif";
      imgEl.src = `${singerFolder}/${baseName}.avif`;
   } else {
      imgEl.onerror = null;
      imgEl.src = defaultAvatar || selectedSingerImg || 'https://cdn-icons-png.flaticon.com/512/461/461238.png';
   }
}
...
function toStep2() {
   ...
   listDiv.innerHTML = "";
   const rawSongs = songData[selectedSingerName] || [];
   rawSongs.forEach((rawSong, index) => {
      const song = getSingerSongMedia(rawSong, selectedSingerName);
      const div = document.createElement('div');
      div.className = 'song-item';
      const thumbImg = song.img || selectedSingerImg;
      div.innerHTML = `
         <img src="${thumbImg}" onerror="handleImageError(this, '${selectedSingerName}', '${song.imageBase}', '${selectedSingerImg}')">
         <div class="song-info">
...
function playSong(songTitle, songUrl, thumb, artist) {
   ...
   const rawSongs = songData[selectedSingerName] || [];
   const playlist = rawSongs.map((rawSong) => {
      const s = getSingerSongMedia(rawSong, selectedSingerName);
      return {
         name: s.title,
         artist: artist,
         cover: s.img || thumb,
...
/* ================= AUTOMATIC LOCAL SONG DISCOVERY =================
   The PHP endpoint scans singers/<singer>/audio/ on every page load.
   This means new audio files added to a singer's folder appear automatically.
   Static hosting without PHP cannot enumerate folders, so the existing songData
   remains as a fallback for those hosts.
*/
async function discoverLocalSongs() {
   try {
      const response = await fetch('scan-songs.php?ts=' + Date.now(), {
         cache: 'no-store',
      ...
      Object.entries(discovered).forEach(([singer, songs]) => {
         if (!Array.isArray(songs) || !songs.length) return;
         if (!songData[singer]) songData[singer] = [];

         const existing = new Set(songData[singer].map(s =>
            String((s && (s.url || s.file || s.filename || s.title)) || '').toLowerCase()
         ));

         songs.forEach(song => {
            const key = String((song.url || song.file || song.filename || song.title) || '').toLowerCase();
            if (!key) return;
            if (!existing.has(key)) {
               songData[singer].push(song);
               existing.add(key);
            } else {
               // Replace a matching local entry so renamed/missing metadata is refreshed.
               const idx = songData[singer].findIndex(s =>
                  String((s && (s.url || s.file || s.filename || s.title)) || '').toLowerCase() === key
               );
               if (idx >= 0 && song.local === true) songData[singer][idx] = song;
            }
         });
      });

      // Refresh the currently open singer list if discovery completed after the UI opened.
      if (selectedSingerName && document.getElementById('step2')?.classList.contains('active')) {
         toStep2();
      }
   } catch (error) {
      // Expected on static hosts that do not execute PHP. Keep the built-in songData fallback.
      console.info('Automatic folder scan unavailable; using built-in song list.', error);
   }
}
