...
/* Helper to resolve local matched song audio/image files or explicit URLs */
function getSingerSongMedia(song, singerName) {
   if (typeof song === 'string') {
      song = { title: song };
   }
   const baseName = song.filename || song.file || song.title;
   const defaultUrl = `singers/${singerName}/audio/${baseName}.mp3`;
   const defaultImg = `singers/${singerName}/images/${baseName}.jpg`;
   
   return {
      title: song.title || baseName,
      url: song.url ? song.url : defaultUrl,
      img: song.img ? song.img : defaultImg,
...
function toStep2() {
   ...
   rawSongs.forEach((rawSong, index) => {
      ...
      div.innerHTML = `
         <img src="${thumbImg}" onerror="this.src='${selectedSingerImg}'">
         <div class="song-info">
